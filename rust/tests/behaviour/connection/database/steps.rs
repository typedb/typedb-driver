...
generic_step_impl! {
    ...
    pub async fn connection_create_database(context: &mut Context, name: String) {
        create_database_with_timeout(context.driver.databases(), name).await;
    }
    ...
    async fn connection_create_databases_in_parallel(context: &mut Context, step: &Step) {
        join_all(util::iter_table(step).map(|name| create_database_with_timeout(context.driver.databases(), name.to_string())))
            .await;
    }

    #[step(expr = "connection delete database: {word}")]
    pub async fn connection_delete_database(context: &mut Context, name: String) {
        context.driver.databases().get(name).and_then(Database::delete).await.unwrap();
    }
    ...
    async fn connection_delete_databases_in_parallel(context: &mut Context, step: &Step) {
        try_join_all(util::iter_table(step).map(|name| context.driver.databases().get(name).and_then(Database::delete)))
            .await
            .unwrap();
    }
    ...
    async fn connection_delete_database_throws_exception(context: &mut Context, name: String) {
        assert!(context.driver.databases().get(name).and_then(Database::delete).await.is_err());
    }
    ...
    async fn connection_has_database(context: &mut Context, name: String) {
        assert_with_timeout!(
            context.driver.databases().contains(name.clone()).await.unwrap(),
