...
public class RoleTypeImpl extends TypeImpl implements RoleType {
    ...
    public final void delete(TypeDBTransaction transaction) {
        role_type_delete(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject);
    }

    @Override
    public final boolean isDeleted(TypeDBTransaction transaction) {
        return role_type_is_deleted(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject);
    }

    @Override
    public final void setLabel(TypeDBTransaction transaction, String newLabel) {
        role_type_set_label(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, newLabel);
    }

    @Nullable
    @Override
    public RoleTypeImpl getSupertype(TypeDBTransaction transaction) {
        com.vaticle.typedb.client.jni.Concept res = role_type_get_supertype(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject);
    ...
    public final Stream<RoleTypeImpl> getSupertypes(TypeDBTransaction transaction) {
        return role_type_get_supertypes(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject).stream().map(RoleTypeImpl::new);
    }

    @Override
    public final Stream<RoleTypeImpl> getSubtypes(TypeDBTransaction transaction) {
        return role_type_get_subtypes(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Transitive).stream().map(RoleTypeImpl::new);
    }

    @Override
    public final Stream<RoleTypeImpl> getSubtypesExplicit(TypeDBTransaction transaction) {
        return role_type_get_subtypes(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Explicit).stream().map(RoleTypeImpl::new);
    }

    @Nullable
    @Override
    public final RelationType getRelationType(TypeDBTransaction transaction) {
        return new RelationTypeImpl(role_type_get_relation_type(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject));
    }

    @Override
    public final Stream<RelationTypeImpl> getRelationTypes(TypeDBTransaction transaction) {
        return role_type_get_relation_types(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject).stream().map(RelationTypeImpl::new);
    }

    @Override
    public final Stream<ThingTypeImpl> getPlayerTypes(TypeDBTransaction transaction) {
        return role_type_get_player_types(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Transitive).stream().map(ThingTypeImpl::of);
    }

    @Override
    public final Stream<ThingTypeImpl> getPlayerTypesExplicit(TypeDBTransaction transaction) {
        return role_type_get_player_types(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Explicit).stream().map(ThingTypeImpl::of);
    }

    @Override
    public final Stream<RelationImpl> getRelationInstances(TypeDBTransaction transaction) {
        return role_type_get_relation_instances(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Transitive).stream().map(RelationImpl::new);
    }

    @Override
    public final Stream<RelationImpl> getRelationInstancesExplicit(TypeDBTransaction transaction) {
        return role_type_get_relation_instances(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Explicit).stream().map(RelationImpl::new);
    }

    @Override
    public final Stream<ThingImpl> getPlayerInstances(TypeDBTransaction transaction) {
        return role_type_get_player_instances(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Transitive).stream().map(ThingImpl::of);
    }

    @Override
    public final Stream<ThingImpl> getPlayerInstancesExplicit(TypeDBTransaction transaction) {
        return role_type_get_player_instances(((ConceptManagerImpl) transaction.concepts()).transaction, nativeObject, Transitivity.Explicit).stream().map(ThingImpl::of);
    }
}
