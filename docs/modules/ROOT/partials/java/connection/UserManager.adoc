...
Retrieves the name of the user who opened the current connection, using default strong consistency. See <<#_getCurrentUser_com_typedb_driver_api_ConsistencyLevel,``getCurrentUser(ConsistencyLevel)``>> for more details and options. 
...
[#_UserManager_getCurrentUser_ConsistencyLevel]
==== getCurrentUser

[source,java]
----
@CheckReturnValue
User getCurrentUser​(ConsistencyLevel consistencyLevel)
             throws TypeDBDriverException
----
