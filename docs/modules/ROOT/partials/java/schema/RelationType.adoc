...
Returns ``true`` if the value which this ``Concept`` holds is of type ``vector`` or if this ``Concept`` is an ``AttributeType`` of type ``vector``. Otherwise, returns ``false``. 
...
[#_RelationType_tryGetVector_]
==== method ``tryGetVector``

[source,java]
----
java.util.Optional<java.util.List<java.lang.Float>> tryGetVector()
----

Returns the value of this ``Concept`` as an ``Optional&lt;List&lt;Float&gt;&gt;``. If it's not a ``Value`` or it has another type, returns ``null``. 
