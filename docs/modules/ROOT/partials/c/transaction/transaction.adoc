...
[#_transaction_query_given_rows]
==== function ``transaction_query_given_rows``
...
a| `query` a| The query string. a| `const char*`
a| `options` a| ``QueryOptions`` to configure the executed query. a| `const struct QueryOptions*`
a| `given_rows` a| ``QueryGivenRows`` the rows given as input to the query. a| `struct GivenRows*`
|===
