...
[#_transaction_drop_sync]
==== function ``transaction_drop_sync``
...
[source,c]
----
struct VoidPromise* transaction_on_close(const struct Transaction* txn, uintptr_t callback_id, void(* callback)(uintptr_t, struct Error*))
----
...
a| `callback` a| The function to be called a| `void(*`
|===
