[#_methods_analyze_pipeline]
=== pipeline

[#_Struct_Pipeline]
==== Struct Pipeline

`Class(name=Struct Pipeline, anchor=Struct_Pipeline, enumConstants=[], description=[, A representation of a query pipeline.], examples=[], fields=[], methods=[], packagePath=null, superClasses=[], traitImplementors=[], mode=null).nameDescriptor(language)`



A representation of a query pipeline.

[#_pipeline_drop]
==== pipeline_drop

[source,cpp]
----
void pipeline_drop(struct Pipeline* obj)
----



Frees the native rust ``Pipeline`` object.

[caption=""]
.Returns
`void`

[#_pipeline_get_conjunction]
==== pipeline_get_conjunction

[source,cpp]
----
struct Conjunction* pipeline_get_conjunction(const struct Pipeline* pipeline, const struct ConjunctionID* conjunction_id)
----



Returns the ``Conjunction`` corresponding to the ConjunctionID. The ``Pipeline`` must be the one that contains the Conjunction &amp; ConjunctionID.

[caption=""]
.Returns
`struct Conjunction*`

