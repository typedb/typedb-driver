...
[#_constraint_delete_concepts_get_variables]
==== function ``constraint_delete_concepts_get_variables``

[source,c]
----
struct ConstraintVertexIterator* constraint_delete_concepts_get_variables(const struct ConstraintWithSpan* constraint)
----



Unwraps the ``Constraint`` instance as a ``DeleteConcepts`` constraint, and returns the variables whose concepts are deleted. Will panic if the Constraint is not a DeleteConcepts constraint.
