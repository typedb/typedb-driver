...
a| `primary_failover_retries` a| `int` a| Specifies the number of retries the driver performs to find and reach the cluster primary after a failed request, before giving up. Total attempts per user request = ``N + 1``. Each retry either follows the server’s redirect address (fast path) or polls the known replicas with a 2-second sleep between polls (slow path). Set to ``0`` to disable failover. Defaults to 1.
