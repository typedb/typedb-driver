...
[#_Transaction_query_query_str_options_QueryOptions_None_given_rows_GivenRows_None]
==== method ``query``

[source,python]
----
query(query: str, options: QueryOptions | None = None, given_rows: GivenRows | None = None) -> Promise[QueryAnswer]
----
...
a| `given_rows` a| Rows given to the query as input. a| `GivenRows \| None` a| `None`
|===
