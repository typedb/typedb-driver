...
[#_Transaction_query_query_str_options_QueryOptions_None_given_rows_]
==== method ``query``

[source,python]
----
query(query: str, options: QueryOptions | None = None, given_rows=None) -> Promise[QueryAnswer]
----
...
a| `given_rows` a| Rows given to the query as input. Will throw an exception if a type is passed. a| `` a| `None`
|===
