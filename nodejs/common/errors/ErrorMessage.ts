...
export namespace ErrorMessage {
    ...
    export namespace Driver {
        ...
        export const CLUSTER_NO_PRIMARY_REPLICA_YET = new Driver(14, (args: Stringable[]) => `No replica has been marked as the primary replica for latest known term '${args[0]}'.`);
        export const CLUSTER_UNABLE_TO_CONNECT = new Driver(15, (args: Stringable[]) => `Unable to connect to TypeDB Cluster. Attempted connecting to the cluster nodes, but none are available: '${args[1]}'.`);
        export const CLUSTER_REPLICA_NOT_PRIMARY = new Driver(16, () => `The replica is not the primary replica.`);
        export const CLUSTER_ALL_NODES_FAILED = new Driver(17, (args: Stringable[]) => `Attempted connecting to all cluster nodes, but the following errors occurred: \n'${args[0]}'`);
        export const USER_MANAGEMENT_CLUSTER_ONLY = new Driver(18, () => `User management is only available in TypeDB Cluster servers.`);
        export const CLUSTER_USER_DOES_NOT_EXIST = new Driver(19, (args: Stringable[]) => `The user '${args[0]}' does not exist.`);
        export const CLUSTER_TOKEN_CREDENTIAL_INVALID = new Driver(20, (args: Stringable[]) => `Invalid token credential.`);
        export const CLUSTER_INVALID_ROOT_CA_PATH = new Driver(21, (args: Stringable[]) => `The provided Root CA path '${args[0]}' does not exist`);
