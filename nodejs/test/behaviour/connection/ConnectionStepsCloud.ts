...
BeforeAll(async () => {
    setDefaultDriverFn(async () =>
        TypeDB.clusterDriver("127.0.0.1:11729", new TypeDBCredential("admin", "password", process.env.ROOT_CA))
    )
    setDriverFn(async (username, password) => {
        return TypeDB.clusterDriver("127.0.0.1:11729", new TypeDBCredential(username, password, process.env.ROOT_CA))
    });
