...
pub(super) fn unwrap_arc<T>(t: Arc<T>) -> *const T {
    let raw = Arc::into_raw(t);
    trace!("Unwrap arc to raw <{}> @ {:?}", std::any::type_name::<T>(), raw);
    raw
}

pub(super) fn wrap_into_arc<T>(raw: *const T) -> Arc<T> {
    trace!("Wrap raw into arc <{}> @ {:?}", std::any::type_name::<T>(), raw);
