...
#[unsafe(no_mangle)]
pub extern "C" fn given_rows_builder_set_index_to_concept(builder: *mut GivenRowsBuilder, index: usize, concept: *const Concept) {
    let result = borrow_mut(builder).active_row().and_then(|mut row| {
        row.set_at(index, to_given_row_entry(borrow(concept), index)?)
    });
...
pub extern "C" fn given_rows_builder_set_index_to_empty(builder: *mut GivenRowsBuilder, index: usize) {
    let result = borrow_mut(builder).active_row().and_then(|mut row| {
        row.set_at(index, GivenRowEntry::Empty)
    });
    if let Err(err) = result {
        record_error(err);
    }
