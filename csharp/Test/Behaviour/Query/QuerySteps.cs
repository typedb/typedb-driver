    ...
    public partial class BehaviourSteps
    {
        ...
        private abstract class AttributeMatcherBase : IConceptMatcher
        {
            ...
            protected bool CheckValue(IAttribute attribute)
            {
                ...
                // Normalize value type name (handle variations like datetime-tz, datetime_tz, datetimetz)
                var normalizedType = valueType.Replace("_", "-").Replace(" ", "-");
                ...
                switch (normalizedType)
                {
                    ...
                    case "decimal":
                        // Strip "dec" suffix if present (TypeDB uses "123.456dec" format)
                        var decValue = Value.EndsWith("dec", StringComparison.OrdinalIgnoreCase)
                            ? Value.Substring(0, Value.Length - 3)
                            : Value;
                        return value.GetDecimal() == decimal.Parse(decValue, System.Globalization.CultureInfo.InvariantCulture);
                    case "string":
                    ...
                    case "datetime":
                        return CompareDateTime(value.GetDatetime(), Value) ||
                               CompareByNativeString(attribute, Value);
                    case "datetime-tz":
                    case "datetimetz":
                        return CompareDateTimeTZ(value.GetDatetimeTZ(), Value) ||
                               CompareByNativeString(attribute, Value);
                    case "duration":
                        return CompareDuration(value.GetDuration(), Value) ||
                               CompareByNativeString(attribute, Value);
                    default:
                        return false;
                }
            ...
            private bool CompareDateTime(DateTime actual, string expected)
            {
                ...
                // TypeDB datetime can have up to nanosecond precision (9 digits after decimal)
                // but C# DateTime only has tick precision (7 digits, 100ns)

                // Try standard parsing first
                if (DateTime.TryParse(expected, System.Globalization.CultureInfo.InvariantCulture,
                    System.Globalization.DateTimeStyles.None, out var parsed))
                {
                    // Compare up to the precision supported by DateTime (ticks = 100ns)
                    return actual.Ticks == parsed.Ticks;
                }

                // Handle high-precision format like "1990-01-01T11:22:33.123456789"
                // Split on the decimal point to extract the fractional part
                var tIdx = expected.LastIndexOf('T');
                if (tIdx > 0)
                {
                    var dotIdx = expected.IndexOf('.', tIdx);
                    if (dotIdx > 0)
                    {
                        // Truncate to 7 digits (tick precision) and try parsing again
                        var fractionalPart = expected.Substring(dotIdx + 1);
                        if (fractionalPart.Length > 7)
                        {
                            var truncated = expected.Substring(0, dotIdx + 8); // +1 for dot, +7 for digits
                            if (DateTime.TryParse(truncated, System.Globalization.CultureInfo.InvariantCulture,
                                System.Globalization.DateTimeStyles.None, out var truncatedParsed))
                            {
                                return actual.Ticks == truncatedParsed.Ticks;
                            }
                        }
                    }
                }
            ...
            private bool CompareDateTimeTZ(DateTimeOffset actual, string expected)
            {
                // TypeDB datetime-tz can have various formats:
                // 1. ISO format with offset: "1990-01-01T11:22:33+0530" or "1990-01-01T11:22:33-0100"
                // 2. Named timezone: "1990-01-01T11:22:33 Asia/Kathmandu"

                // Check for named timezone format (has space after the datetime, followed by timezone name)
                // Named timezones don't start with + or - after the space
                var lastSpace = expected.LastIndexOf(' ');
                if (lastSpace > 0)
                {
                    var afterSpace = expected.Substring(lastSpace + 1);
                    // If what's after the space is not a numeric offset, it's a timezone name
                    if (afterSpace.Length > 0 && afterSpace[0] != '+' && afterSpace[0] != '-' && !char.IsDigit(afterSpace[0]))
                    {
                        // Named timezone format like "1990-01-01T11:22:33 Asia/Kathmandu"
                        var dtPart = expected.Substring(0, lastSpace);
                        // For named timezones, just compare the local datetime component
                        // since C# doesn't directly support IANA timezone parsing without extra libs
                        if (DateTime.TryParse(dtPart, out var dtParsed))
                        {
                            return actual.DateTime == dtParsed;
                        }
                    }
                }

                // Try ISO format with offset (e.g., "1990-01-01T11:22:33-0100")
                // The format might not have the colon in the offset
                var normalizedExpected = expected;
                // Check if offset is in compact form without colon (e.g., "-0100" instead of "-01:00")
                if (normalizedExpected.Length > 5)
                {
                    var lastPart = normalizedExpected.Substring(normalizedExpected.Length - 5);
                    if ((lastPart[0] == '+' || lastPart[0] == '-') &&
                        char.IsDigit(lastPart[1]) && char.IsDigit(lastPart[2]) &&
                        char.IsDigit(lastPart[3]) && char.IsDigit(lastPart[4]))
                    {
                        // Insert colon: "-0100" -> "-01:00"
                        normalizedExpected = normalizedExpected.Substring(0, normalizedExpected.Length - 2) +
                                           ":" + normalizedExpected.Substring(normalizedExpected.Length - 2);
                    }
                }

                if (DateTimeOffset.TryParse(normalizedExpected, System.Globalization.CultureInfo.InvariantCulture,
                    System.Globalization.DateTimeStyles.None, out var parsed))
                {
            ...
            private bool CompareDuration(Duration actual, string expected)
            {
            ...
            /// <summary>
            /// Fallback comparison using the native concept_to_string representation.
            /// This handles cases where precision or formatting differs between C# and TypeDB.
            /// </summary>
            private bool CompareByNativeString(IAttribute attribute, string expectedValue)
            {
                // The attribute's ToString() returns the native string representation from concept_to_string
                var attrString = attribute.ToString();

                // Check if the native string contains the expected value (might be prefixed with type info)
                if (attrString.Contains(expectedValue))
                    return true;

                // For datetime with nanoseconds, the native format might differ slightly
                // Try normalizing both strings for comparison
                var normalizedExpected = NormalizeDateTimeString(expectedValue);
                if (!string.IsNullOrEmpty(normalizedExpected) && attrString.Contains(normalizedExpected))
                    return true;
            ...
            /// <summary>
            /// Normalize datetime string for comparison (handles precision differences).
            /// </summary>
            private string NormalizeDateTimeString(string dt)
            {
                // Remove trailing zeros from fractional seconds for comparison
                // e.g., "1990-01-01T11:22:33.123456789" might be stored as "1990-01-01T11:22:33.12345679"
                // due to floating point representation
                if (dt.Contains("T") && dt.Contains("."))
                {
                    var dotIdx = dt.LastIndexOf('.');
                    var fractionalEnd = dt.Length;
                    for (int i = dotIdx + 1; i < dt.Length; i++)
                    {
                        if (!char.IsDigit(dt[i]))
                        {
                            fractionalEnd = i;
                            break;
                        }
                    }
                    // Return the string up to and including the decimal portion
                    return dt;
                }
                return dt;
            }
